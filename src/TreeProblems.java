...
public class TreeProblems {
  ...
  public static <T> void postOrder(Node<T> root) {
    if(root == null) return;
    for(Node<T> child : root.children) { postOrder(child); }
  ...
  public static <T> void postOrder(Map<T, List<T>> tree, T root) {
    ...
    for(T child : children) { postOrder(tree, child); }
  ...
  */
  public static int sumTree(Map<Integer, List<Integer>> tree) {
    ...
    int sum = 0;
    for(int key : tree.keySet()) { sum += key; }
    return sum;
  }
  ...
  */
  public static <T> T findRoot(Map<T, List<T>> tree) {
    Set<T> allChilds = new HashSet<>();
    for(List<T> children : tree.values()) { allChilds.addAll(children); }
    for(T node : tree.keySet()) { if(!allChilds.contains(node)) { return node; } }
    return null;
  }
  ...
  */
  public static <T> int maxDepth(Node<T> root) {
    if(root == null) return 0;
    int max = 0;
    for(Node<T> child : root.children) { max = Math.max(max, maxDepth(child)); }
    return max + 1;
  }
  ...
  */
  ...
  private static int maxDepthHelper(Map<String, List<String>> tree, String node) {
    if(tree.get(node).isEmpty()) return 1;
    int max = 0;
    for(String child : tree.get(node)) { max = Math.max(max, maxDepthHelper(tree, child)); }
    return max + 1;
  }
}
